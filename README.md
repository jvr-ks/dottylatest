...
```
ThisBuild / scalaVersion := sys.env.get("DOTTYLATEST").getOrElse("3.6.2"),  
```
or  
```
inThisBuild(
	List(
		scalaVersion := sys.env.get("DOTTYLATEST").getOrElse("3.6.2")
	)
)
```  
...
inThisBuild(
	List(
		scalaVersion := sys.env.get("DOTTYLATESTRC").getOrElse("3.6.2")
	)
)
``` 
...
- [scala3-library_3-3.6.2.jar](https://repo1.maven.org/maven2/org/scala-lang/scala3-library_3/3.6.2/scala3-library_3-3.6.2.jar )  
