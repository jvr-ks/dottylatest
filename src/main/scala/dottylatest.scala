...
object Dottylatest:
  ...
  def readURL(): Either[String, String] = 
    ...
    try
      val connection = (new URL(repoURL)).openConnection.asInstanceOf[HttpURLConnection]
  ...
  val extractDate: Either[String, String] => Either[String, (String, String)] = {
    case Right(value) => 
      val regexDate = """-bin-(\d+)""".r
      val latestDate = (regexDate findAllIn value).matchData.map(_ group 1).toList.sorted.last
      if (latestDate.size > 1)
        Right((latestDate, value))
      else
        Left("ERROR, no suitable entry found in content of the webpage!")
    case Left(e) => Left(e)
  }
      
  val extractNameAndSet: (Either[String, (String, String)])  => Unit = {
    case Right(value) => 
      val regexName = ("""<a href="(.+?""" + value._1 + """.+?NIGHTLY)/"""").r
      val latestDottyVersion = (regexName findAllIn value._2).matchData.map(_ group 1).toList.sorted.last
      if (latestDottyVersion.size > 1)
        osName match
          case "win" =>
  ...
  def main(args: Array[String]): Unit =
    extractNameAndSet(extractDate(readURL()))
    sys.exit(0)
